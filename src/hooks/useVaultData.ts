...
export function isCCPayment(notes: string): boolean {
  return notes?.toLowerCase().includes('cc bill payment') ?? false
}

export function extractFee(notes: string): number {
  if (!notes) return 0
...
export function computeVaultData(csvText: string): VaultData {
...
export function useVaultData(csvText: string | null): VaultData | null {
  return useMemo(() => {
    ...
    return computeVaultData(csvText)
  }, [csvText])
}
